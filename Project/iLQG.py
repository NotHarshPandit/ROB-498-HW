...
class ILQGController:
    ...
    def backward(self, x_seq, u_seq):
        ...
        for t in range(self.pred_time - 1, -1, -1):
            ...
            inv_q_uu_hat = np.linalg.inv(q_uu_hat)
            k = -np.matmul(inv_q_uu_hat, q_u)
            K = -np.matmul(inv_q_uu_hat, q_ux_hat)
            delta_v = (0.5 * k.T @ q_uu @ k)+ np.matmul(q_u, k)
            self.v[t] += delta_v
            self.v_x[t] = q_x + (K.T @ q_uu @ k)+ K.T @ q_u + q_ux.T@k
            self.v_xx[t] = q_xx + (K.T @ q_uu @ K)+ K.T @ q_ux + q_ux.T@k
            k_seq.append(k)
            K_seq.append(K)
        k_seq.reverse()
        K_seq.reverse()
