...
def collect_data_random_trajectory(env, num_trajectories=1000, trajectory_length=10):
    ...
    collected_data = []
    # action_space,state_space = env.action_space,env.observation_space
    ...
    for trajectory in tqdm(range (num_trajectories)):
        ...
        for _ in range(trajectory_length):
            ...
            state_list.append(state.astype(np.uint8))
...
def process_data_multiple_step(collected_data, batch_size=500, num_steps=4):
   ...
   # create a dataset from collected data
    ...
    # train_mean = torch.mean(train_states)
    # train_std = torch.std(train_states)
...
class MultiStepDynamicsDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        sample_index = item % self.trajectory_length
        sample['states'] = torch.from_numpy(self.data[trajectory_index]['states'][sample_index:sample_index+self.num_steps+1]).permute((0,3,1,2)).to(dtype=torch.float32)
        sample['actions'] = torch.from_numpy(self.data[trajectory_index]['actions'][sample_index:sample_index+self.num_steps]).to(dtype=torch.float32)
        # ---
        if self.transform:
...
class StateEncoder(nn.Module):
    ...
    def forward(self, state):
        ...
        # --- Your code here
        latent_state = self.encoder_layers(state)
        # ---
...
class StateDecoder(nn.Module):
    ...
    def forward(self, latent_state):
        ...
        # --- Your code here
        decoded_state = self.decoder_layers(latent_state)
        # ---
...
class StateVAE(nn.Module):
    ...
    def encode(self, state):
        ...
        latent_state = None
        # --- Your code here
        latent_state = self.encoder.encoder_layers(state)
        latent_state = self.encoder.linear_mu(latent_state)
        # ---
        return latent_state
...
class LatentDynamicsModel(nn.Module):
    ...
    def encode(self, state):
        ...
        latent_state = None
        # --- Your code here
        # passing the state throught encoder
...
def img_space_pushing_cost_function(state, action, target_state):
    ...
    cost = None
    # --- Your code here
    # Expand target_state to shape (B, C, H, W) to match state
    target_state_expanded = target_state.unsqueeze(0).expand_as(state)
    
    # Compute mean squared error per sample (no reduction across batch)
    cost = F.mse_loss(state, target_state_expanded, reduction='none')  # shape: (B, C, H, W)
    
    # Average over channel, height, width dimensions to get per-sample cost
    cost = cost.mean(dim=[1, 2, 3])  # shape: (B,)
...
class PushingImgSpaceController(object):
    ...
    def control(self, state):
        ...
        state_tensor_norm = (state_tensor - self.norm_constants['mean']) / self.norm_constants['std']
        state_tensor_wrapped = self._wrap_state(state_tensor_norm)

        # ---
        action_tensor = self.mppi.command(state_tensor_wrapped)
        # --- Your code here
    ...
    def _wrap_state(self, state):
        ...
        wrapped_state = None
        # --- Your code here
        wrapped_state = state.reshape(*state.shape[:-3], -1)
        # ---
    ...
    def _unwrap_state(self, wrapped_state):
        ...
        state = None
        # --- Your code here
        height, width, num_channels = self.env.observation_space.shape
