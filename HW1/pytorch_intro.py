...
from torch import Tensor, triangular_solve
...
def compute_vector_function_and_grad(x: Tensor) -> Tensor:
    ...
    y = None
    # --- Your code here
    x.requires_grad_(True)  # Ensure x is a leaf tensor with grad tracking
    y1 = torch.cos(2 * x[0] + x[1])
    y2 = torch.sin(2 * x[1] - x[0])
    y = torch.stack([y1, y2])  # Ensures proper autograd tracking
...
def compute_scalar_function_and_partial_grad(x: Tensor, y: Tensor) -> Tensor:
    ...
    with torch.no_grad():  
        y = y.clone().detach()  # Ensure y does NOT track gradients
    z = (x**0.5)*y
    z.backward()
    y.grad = None
    # ---
...
def compute_forward_kinematics(thetas: torch.Tensor) -> torch.Tensor:
    ...
    x = None
    # --- Your code here
    x = torch.tensor([L1*torch.cos(thetas[0]) + L2*(torch.cos(thetas[0] + thetas[1])) ,
                      L1*torch.sin(thetas[0]) + L2*(torch.sin(thetas[0] + thetas[1]))])
    # ---
    return x
...
def compute_jacobian(thetas: torch.Tensor) -> torch.Tensor:
    ...
    L1 = 2
    L2 = 1
    J =  torch.tensor([[-L1*torch.sin(thetas[0]) - L2*(torch.sin(thetas[0] + thetas[1])) , -L2*torch.sin(thetas[0]+thetas[1])],
                       [L1*torch.cos(thetas[0]) + L2*torch.cos(thetas[0]+thetas[1]) , L2*torch.cos(thetas[0]+thetas[1])]])
    # ---
    return J
