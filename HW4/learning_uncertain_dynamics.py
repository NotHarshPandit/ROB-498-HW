...
class SingleStepDynamicsDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        traj_index = item // self.trajectory_length #determine which trajectory the sample belongs to
        time_index = item % self.trajectory_length #determine which time step in the trajectory
        # --- Your code here
        # print(self.data)
        sample['state'] = self.data[traj_index]['states'][time_index]
        sample['action'] = self.data[traj_index]['actions'][time_index]
        sample['next_state'] = self.data[traj_index]['states'][time_index+1]
...
class PushingDynamics(nn.Module):
    ...
    def propagate_uncertainty_moment_matching(self, mu, sigma, action, K=50):
        ...
        # 
        batch_size, state_size = mu.shape 
        # --- Your code here
        pred_mu, pred_sigma = None, None
        states  = []
        for i in range(K):
            next_mu_i, next_sigma_i = self.propagate_uncertainty_certainty_equivalence(mu,sigma,action)
            distribution = torch.distributions.MultivariateNormal(next_mu_i,next_sigma_i)
            state_i = distribution.sample()
            states.append(state_i)
        states = torch.stack(states,dim=1)
        pred_mu = torch.mean(states,dim=1)
        diffs = (states - pred_mu.unsqueeze(1)).reshape(batch_size * K, state_size)
        prods = torch.bmm(diffs.unsqueeze(2), diffs.unsqueeze(1)).reshape(batch_size, K, state_size,state_size)
...
class PushingDynamicsGP(PushingDynamics):
    ...
    def forward(self, state, action):
        ...
        pred = None
        # --- Your code here
        # mean_x = self.gp_model.mean_module(torch.cat((state, action), dim=1))
        # covar_x = self.gp_model.covar_module(torch.cat((state, action), dim=1))
        # pred = gpytorch.distributions.MultitaskMultivariateNormal.from_batch_mvn(
        #     gpytorch.distributions.MultivariateNormal(mean_x, covar_x)
        # )
    ...
    def propagate_uncertainty_linearization(self, mu, sigma, action):
        ...
        # --- Your code here
        pred_mu,pred_sigma = self.predict(mu,action)
        pred_sigma = A @ sigma @ A.transpose(1,2) + pred_sigma
...
class ResidualDynamicsModel(nn.Module):
    ...
    def forward(self, state, action):
        ...
        next_state = None
        # --- Your code here
        next_state = state + self.layers(torch.cat((state,action),dim=-1))
        # ---
        return next_state
...
class DynamicsNNEnsemble(PushingDynamics):
    ...
    def predict(self, state, action):
        ...
        pred_mu, pred_sigma = None, None
        # --- Your code here
        pred_state = self.forward(state,action) #predict the next state
        pred_mu = torch.mean(pred_state, dim = 1)
        pred_sigma =  batch_cov(pred_state)
        # ---
        return pred_mu, pred_sigma
...
def train_dynamics_gp_hyperparams(model, likelihood, train_states, train_actions, train_next_states, lr):
    ...
    """
    # --- Your code here
    # if isinstance(model, PushingDynamicsGP):
    #     training_iter = 50
    # else:
    #     training_iter = 300
    training_iter = 500
    ...
    for i in tqdm(range(training_iter)):
        ...
        loss_i = -mll(output, train_next_states)
        loss_i.backward()
        optimizer.step()
...
def obstacle_avoidance_pushing_cost_function(state, action):
    ...
    def in_collision(mu, obstacle):
      distance = torch.norm(mu-obstacle,dim=-1)
      return ((distance - obs_radius - disk_radius) < 0).to(dtype=torch.float32)

    mu = state[:,:2]
    sigma = state[:,2:].reshape(-1,2,2)
    state_error = mu - target_pose[:2]
    Q = torch.tensor([[1.0,0.0],[0.0,1.0]])
    cost = torch.sum((state_error @ Q ) * state_error, dim = -1) + torch.sum((sigma @ Q).diagonal(0,1,2),dim=-1) + 100*in_collision(mu,obs_centre)
    # ---
    return cost
...
def obstacle_avoidance_pushing_cost_function_samples(state, action, K=10):
    ...
    state_error = mu - target_pose[:2]
    Q = torch.tensor([[1.0,0.0],[0.0,1.0]])
    cost = torch.sum((state_error @ Q ) * state_error, dim = -1) + torch.sum((sigma @ Q).diagonal(0,1,2),dim=-1) + 100*in_collision_samples(mu,sigma,obs_centre)   
